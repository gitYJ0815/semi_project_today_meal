...
<meta charset="UTF-8">
<title>상품 등록</title>
<link href="<%= request.getContextPath() %>/resources/css/product/productUpdateView.css" rel="stylesheet">
		...
		<div class="contents">
			...
			<div class="inner">
				<div class="product_title">
			 	...
			 	<div class="subject_category">
				 	...
				 	<select name="category" id="category">
 						...
 						<option value="1" <c:if test="${ product.cNo == 1 }">selected</c:if>>한식</option>
 						<option value="2" <c:if test="${ product.cNo == 2 }">selected</c:if>>양식</option>
 						<option value="3" <c:if test="${ product.cNo == 3 }">selected</c:if>>중식</option>
 						<option value="4" <c:if test="${ product.cNo == 4 }">selected</c:if>>일식</option>
 						<option value="5" <c:if test="${ product.cNo == 5 }">selected</c:if>>분식</option>
 						<option value="6" <c:if test="${ product.cNo == 6 }">selected</c:if>>야식</option>
 						<option value="7" <c:if test="${ product.cNo == 7 }">selected</c:if>>샐러드</option>
 					</select>			
 				</div>
 				<div class="subject_title">
				 	<div class="title_div">상품명<b>*</b></div>
				 	<input type="text" class="title" name="title" value="${ product.pName }" required>
				</div>
				...
				<div id="product_thumbnail">
					<!-- 대표이미지 담는 div -->
					<img id="pro_thumbnail" src="${ contextPath }/resources/uploadFiles/product/${ product.pImg }">
				</div>
				<div class="subject_price">
				 	<div class="price_div">금액<b>*</b></div>
				 	<input type="text" name="price" value="${ product.pPrice }" required>&nbsp;원
				</div>
